    ...
    : m_model(model)
{
    if (mass == STATIONARY) {
        ...
        m_inverse_moment_of_inertia =
            m_inverse_mass / ((2.0f / 5.0f) * m_model->radius() * m_model->radius());
    }
}

namespace {
void
copy_model_to_vectors(const Physics_model& model, Math::Coordinate_space coords,
