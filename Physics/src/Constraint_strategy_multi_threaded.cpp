...
void Constraint_strategy_multi_threaded::warm_start( std::map<std::tuple<Physics_object*,Physics_object*>, Contact_manifold> &manifolds )
{
    ...
    for (auto &manifold : manifolds) {
        ...
        if (info.size() >= MANIFOLD_PER_THREAD) {
            futures.push_back( std::async( std::launch::async, &Constraint_strategy_multi_threaded::warm_start_internal, this, std::move(info) ) );
            info.clear();
        }
    }
    futures.push_back( std::async( std::launch::async, &Constraint_strategy_multi_threaded::warm_start_internal, this, std::move(info) ) );
...
void Constraint_strategy_multi_threaded::solve( int iterations, std::map<std::tuple<Physics_object*,Physics_object*>, Contact_manifold> &manifolds )
{
    for (int i=0; i<iterations; ++i) {
        ...
        for (auto &manifold : manifolds) {
            ...
            info.back().manifold = &manifold.second;
            if (info.size() >= MANIFOLD_PER_THREAD) {
                futures.push_back( std::async( std::launch::async, &Constraint_strategy_multi_threaded::solve_internal, this, std::move(info) ) );
                info.clear();
            }
        }
        futures.push_back( std::async( std::launch::async, &Constraint_strategy_multi_threaded::solve_internal, this, std::move(info) ) );
