...
void
Physics_model::construct(const Math::Local_vector& offset, const Utility::Ac3d_model& model)
{
    ...
    for (const auto& kid : model.kids()) {
        ...
        m_radius =
            std::max(m_radius, Math::to_vector(kid->offset()).length() + m_kids.back()->radius());
    }
}
