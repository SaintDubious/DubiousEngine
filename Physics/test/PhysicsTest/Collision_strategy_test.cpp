...
{		
TEST_CLASS(Collision_strategy_test) {
public:
	TEST_METHOD(collision_strategy_simple)
    {
        ...
        Collision_strategy_simple strategy( 0.05f );
	...
	TEST_METHOD(collision_strategy_multi_threaded)
    {
        std::vector<std::shared_ptr<Physics_object>> objects;
        std::map<Collision_strategy::Physics_object_pair, Contact_manifold> manifolds;
        Collision_strategy_multi_threaded strategy( 0.05f, 4 );
        setup_objects( objects );
        strategy.find_contacts( objects, manifolds );
        Assert::IsTrue( verify_result( objects, manifolds ) );
    }
	...
	TEST_METHOD(collision_strategy_open_cl)
    {
        std::vector<std::shared_ptr<Physics_object>> objects;
        std::map<Collision_strategy::Physics_object_pair, Contact_manifold> manifolds;
        Collision_strategy_open_cl strategy( 0.05f, 4, 8 );
        setup_objects( objects );
        strategy.find_contacts( objects, manifolds );
        Assert::IsTrue( verify_result( objects, manifolds ) );
    }
...
private:
    ...
    bool verify_result( const std::vector<std::shared_ptr<Physics_object>>& objects,
                        const std::map<Collision_strategy::Physics_object_pair, Contact_manifold>& manifolds )
    {
        return manifolds.size() == 10 &&
               // The first index of every sub-sub-vector collides
               verify_pair( 0, 4,   objects, manifolds ) &&
               verify_pair( 0, 8,   objects, manifolds ) &&
               verify_pair( 0, 12,  objects, manifolds ) &&  
               verify_pair( 4, 8,   objects, manifolds ) &&   
               verify_pair( 4, 12,  objects, manifolds ) &&   
               verify_pair( 8, 12,  objects, manifolds ) &&
               // The second and third collide with each other
               verify_pair( 1, 2,   objects, manifolds ) &&
               verify_pair( 5, 6,   objects, manifolds ) &&
               verify_pair( 9, 10,  objects, manifolds ) &&
               verify_pair( 13, 14, objects, manifolds );
    }

    bool verify_pair( size_t a, size_t b, 
