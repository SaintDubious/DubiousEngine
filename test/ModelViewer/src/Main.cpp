...
void OnIdle()
{
//	Math::Rotation YAxis( Math::Vector( 0, 1.0f, 0 ), 0.01f );
//	pObject->CoordinateSpace().Rotate( YAxis );
	pScene->Render( pCamera );

	/*
	if( RotateLight ){
		LightCoords.Rotate( GlobalRotation( 0, 0.01f, 0 ) );
	}
	GlobalVector X, Y, Z;
	LightCoords.Axes( X, Y, Z );
	pScene->SetLight( X*LIGHT_HEIGHT,
