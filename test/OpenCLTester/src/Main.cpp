...
const char* program_source =
    ...
    "        return;\n"
    "    }\n"
    "    int a_i = (num_elements-0.5) - sqrt((num_elements * (num_elements-1)  - (2 * global_id)) "
    "+ 0.25 );\n"
...
main(int argc, char** argv)
{
    try {
        ...
        buffer_result_a =
            clCreateBuffer(context, CL_MEM_WRITE_ONLY, comparison_count * sizeof(int), NULL, &rc);
        if (CL_SUCCESS != rc) {
        ...
        rc = clEnqueueNDRangeKernel(command_queue, kernel, 1, NULL, &global_work_size, NULL, 0,
                                    NULL, NULL);
        if (CL_SUCCESS != rc) {
            std::cout << "rc = " << rc << "\n";
        ...
        clEnqueueReadBuffer(command_queue, buffer_result_a, CL_TRUE, 0,
                            comparison_count * sizeof(int), result_a, 0, NULL, NULL);
        if (CL_SUCCESS != rc) {
            std::cout << "rc = " << rc << "\n";
            throw std::runtime_error("clEnqueueNDRangeKernel");
        }
        int* result_b = new int[comparison_count];
        clEnqueueReadBuffer(command_queue, buffer_result_b, CL_TRUE, 0,
                            comparison_count * sizeof(int), result_b, 0, NULL, NULL);
        if (CL_SUCCESS != rc) {
            std::cout << "rc = " << rc << "\n";
            throw std::runtime_error("clEnqueueNDRangeKernel");
        }
