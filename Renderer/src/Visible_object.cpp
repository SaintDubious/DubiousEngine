...
Visible_object::Visible_object(const std::shared_ptr<Visible_model>& model,
                               const std::shared_ptr<Visible_model>& shadow_model)
    : m_model(model), m_shadow_model(shadow_model), m_base_color(model->color())
{
}

namespace {
void
build(const Visible_model& model, const Math::Local_point& light_pos,
...
void
Visible_object::build_silhouette(const Math::Local_point& light_pos, Silhouette& sil) const
{
    sil.edges.clear();
    sil.kids.clear();
    if (!m_shadow_model)
