...
Visible_model::Visible_model() : m_color(Color::WHITE)
{
}
...
namespace {
Visible_model::surface
build_surface(const std::vector<Math::Local_point>& points, int p0, int p1, int p2)
{
...
void
Visible_model::construct(const Utility::Ac3d_model&                 ac3d_model,
...
void
Visible_model::compute_point_normals()
{
...
Visible_model::build_edges()
{
    ...
    for (size_t i = 0; i < m_surfaces.size(); ++i) {
        ...
        for (j = i + 1; j < m_surfaces.size(); ++j) {
            if (((m_surfaces[j].p0 == i0) &&
                 ((m_surfaces[j].p1 == i1) || (m_surfaces[j].p2 == i1))) ||
                ((m_surfaces[j].p1 == i0) &&
                 ((m_surfaces[j].p0 == i1) || (m_surfaces[j].p2 == i1))) ||
                ((m_surfaces[j].p2 == i0) &&
                 ((m_surfaces[j].p0 == i1) || (m_surfaces[j].p1 == i1)))) {
...
void
Visible_model::add_edge(unsigned short p0, unsigned short p1, unsigned short s0, unsigned short s1)
{
    Edge new_edge;
    new_edge.p0 = p0;
    new_edge.p1 = p1;
