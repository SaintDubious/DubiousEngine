...
Scene::Scene(const std::shared_ptr<Open_gl_context_store>& store) : m_context_store(store)
{
}
...
Scene::render(Camera& camera)
{
    ...
    Open_gl_attributes attribs(Open_gl_attributes::ENABLE_BIT |
                                   Open_gl_attributes::DEPTH_BUFFER_BIT |
                                   Open_gl_attributes::POLYGON_BIT,
                               true);
    ...
    for (auto& object : m_objects) {
        ...
        if (renderer) {
            renderer->render_shadow_volume(object,
                                           Math::to_point(object->coordinate_space().transform(
                                               Math::to_vector(m_scene_light.position))));
        }
    }
...
void
Scene::add_object(const std::shared_ptr<Visible_object>& object)
{
    m_objects.push_back(object);
}

void
Scene::remove_object(const std::shared_ptr<Visible_object>& object)
{
...
void
Scene::setup_light(Open_gl_attributes& attribs, bool in_shadow) const
{
