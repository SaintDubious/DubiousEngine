...
Open_gl_attributes::Open_gl_attributes(int bit_field, bool restore)
    : m_restore(restore), m_group_bits(bit_field)
{
    if (m_restore)
...
void
Open_gl_attributes::clear(int group_bit_field)
{
    glClear(group_bit_field);
}

void
Open_gl_attributes::enable(Attr_bits bit_field) const
{
#ifdef _DEBUG
...
void
Open_gl_attributes::disable(Attr_bits bit_field) const
{
#ifdef _DEBUG
    verify_bits(bit_field);
#endif
...
void
Open_gl_attributes::light_position(int light, const Math::Point& position) const
{
...
void
Open_gl_attributes::light_color(int light, int type, const Color& color) const
{
    float light_params[4];
...
void
Open_gl_attributes::verify_bits(Attr_bits bit_field) const
{
    if (m_group_bits == ALL_ATTRIB_BITS)
        return;
    if (!(m_group_bits & ENABLE_BIT)) {
        if (bit_field == TEXTURE_2D || bit_field == BLEND || bit_field == LIGHTING ||
            bit_field == SCISSOR_TEST || bit_field == STENCIL_TEST) {
            assert(false);
