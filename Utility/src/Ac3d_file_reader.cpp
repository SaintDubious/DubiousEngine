...
// at the moment.
Ac3d_material
read_material(std::ifstream& Input)
{
    std::string Str;
    float       Junk;
    float       Red, Green, Blue;
    Input >> Str >> Str >> Red >> Green >> Blue >> Str >> Junk >> Junk >> Junk >> Str >> Junk >>
        Junk >> Junk >> Str >> Junk >> Junk >> Junk >> Str >> Junk >> Str >> Junk;
...
void
read_surfaces(std::ifstream& input, std::vector<Ac3d_model::Surface>& surfaces)
{
    int num_surfaces;
    input >> num_surfaces;
...
std::unique_ptr<Ac3d_model>
read_model(std::ifstream& input, bool& loc_present)
{
...
std::unique_ptr<const Ac3d_file>
Ac3d_file_reader::read_file(const File_path& file_name)
{
...
namespace {
Ac3d_model::Surface
build_surface(int p0, int p1, int p2)
{
    Ac3d_model::Surface new_surface;
...
std::unique_ptr<Ac3d_model>
build_cube(float width, float height, float depth)
{
    std::unique_ptr<Ac3d_model> model = std::make_unique<Ac3d_model>();
    // The code for this comes from looking at an AC3D file
    // for a cube.
    model->points().push_back(Math::Local_point(-width, -height, -depth));
    model->points().push_back(Math::Local_point(width, -height, -depth));
    model->points().push_back(Math::Local_point(width, -height, depth));
    model->points().push_back(Math::Local_point(-width, -height, depth));
    model->points().push_back(Math::Local_point(-width, height, depth));
...
std::unique_ptr<const Ac3d_file>
Ac3d_file_reader::test_cube(float width, float height, float depth)
{
...
std::unique_ptr<const Ac3d_file>
Ac3d_file_reader::test_cube_group(float edge_length)
{
    std::unique_ptr<Ac3d_model> model = std::make_unique<Ac3d_model>();
    std::unique_ptr<Ac3d_model> child = build_cube(edge_length, edge_length, edge_length);
    child->offset()                   = Math::Local_point(1.0f, 0, 0);
    model->push_kid(std::move(child));
    child           = build_cube(edge_length, edge_length, edge_length);
