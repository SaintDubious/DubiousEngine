...
std::tuple<bool, cl_platform_id, cl_device_id>
Open_cl::setup()
{
    cl_int         rc;
    ...
    for (cl_uint i = 0; i < num_platforms; ++i) {
        ...
        for (cl_uint j = 0; j < num_devices; ++j) {
            ...
            cl_uint uint_value;
            rc = clGetDeviceInfo(device_ids[j], CL_DEVICE_MAX_COMPUTE_UNITS, sizeof(cl_uint),
                                 &uint_value, &returned_size);
            if (CL_SUCCESS != rc) {
                return std::make_tuple(false, nullptr, nullptr);
            }
            std::cout << "\tCL_DEVICE_MAX_COMPUTE_UNITS:  " << uint_value << "\n";
            cl_ulong ulong_value;
            rc = clGetDeviceInfo(device_ids[j], CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(cl_ulong),
                                 &ulong_value, &returned_size);
            if (CL_SUCCESS != rc) {
                return std::make_tuple(false, nullptr, nullptr);
            }
            std::cout << "\tCL_DEVICE_GLOBAL_MEM_SIZE:  " << ulong_value << "\n";
            rc = clGetDeviceInfo(device_ids[j], CL_DEVICE_LOCAL_MEM_SIZE, sizeof(cl_ulong),
                                 &ulong_value, &returned_size);
            if (CL_SUCCESS != rc) {
                return std::make_tuple(false, nullptr, nullptr);
            }
            ...
            size_t size_t_value;
            rc = clGetDeviceInfo(device_ids[j], CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(size_t),
                                 &size_t_value, &returned_size);
            if (CL_SUCCESS != rc) {
                return std::make_tuple(false, nullptr, nullptr);
            }
...
cl_context
Open_cl::create_context(cl_platform_id platform_id, cl_device_id device_id)
{
...
cl_command_queue
Open_cl::create_command_queue(cl_context context, cl_device_id device_id)
{
    cl_int           rc;
...
cl_program
Open_cl::create_program(const char* source, cl_context context, cl_device_id device_id)
{
    cl_int     rc;
    cl_program program;
    std::cout << source << "\n";
...
cl_kernel
Open_cl::create_kernel(cl_program program, const char* kernel_name)
{
    cl_int    rc;
    cl_kernel kernel;
...
cl_mem
Open_cl::create_buffer(cl_context context, cl_mem_flags flags, size_t size)
{
    cl_int rc;
...
void
Open_cl::set_kernel_arg(cl_kernel kernel, cl_uint index, size_t size, const void* arg)
{
...
void
Open_cl::enqueue_write_buffer(cl_command_queue command_queue, cl_mem buffer, cl_bool blocking,
...
void
Open_cl::enqueue_read_buffer(cl_command_queue command_queue, cl_mem buffer, cl_bool blocking,
                             size_t size, void* ptr)
{
    cl_int rc =
        clEnqueueReadBuffer(command_queue, buffer, blocking, 0, size, ptr, 0, nullptr, nullptr);
    if (CL_SUCCESS != rc) {
...
void
Open_cl::enqueue_nd_range_kernel(cl_command_queue command_queue, cl_kernel kernel,
                                 size_t* global_work_size, size_t* local_work_size)
{
    cl_int rc = clEnqueueNDRangeKernel(command_queue, kernel, 1, nullptr, global_work_size,
