...
void
Coordinate_space::rotate(const Quaternion& diff)
{
    m_rotation = diff * m_rotation;
}

void
Coordinate_space::rotate(const Local_quaternion& diff)
{
...
Coordinate_space::transform(const Vector& v) const
{
    ...
    return Local_vector(qrotated.m_imaginary.m_x, qrotated.m_imaginary.m_y,
                        qrotated.m_imaginary.m_z);
}
...
{
    return a.m_position == b.m_position && a.m_rotation == b.m_rotation;
}
